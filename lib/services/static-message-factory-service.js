...
import * as ColorTypes from '../../src/constants/colors';

export const createTransactionToastMessage = ({ status, txnHash }) => {
  const message = `Transaction ${
    status === StatusTypes.SUCCESS || status === StatusTypes.DAPP
      ? 'was successful'
      : 'failed'
  ...
  const modifiedTransactions = transactions.map((txn) => {
    ...
    if (seconds >= 1) {
      if (seconds === 1) {
        ...
        newModifiedTime = moment(new Date(txn.block_timestamp * 1000)).format(
          'MMMM DD,YYYY'
        );
      }
    } else {
...
export const onCreateAccount = (newAlias) =>
  `${newAlias} created successfully.`;

export const onRemoveAccount = (newAlias) =>
  `${newAlias} removed successfully.`;

export const onUpdateCurrentAccount = (newAlias) =>
  `${newAlias} selected as successfully.`;
